...
def batching_data(scores_df, schedule_df, start_time, end_time):
...
def ingest_raw_data(spark: SparkSession, input_path: str, output_path: str, week_itr: int = 0, batch: bool = False):
    ...
    scores_df, schedule_df,  = batching_data(scores_df=scores_df,  schedule_df=schedule_df, start_time=start_time, end_time=end_time)    
    ...
    if batch:
         ...
         schedule_df.write.format("delta").partitionBy("updated_at").mode("append").save(
            os.path.join(output_path, "bronze/schedule_hist")
        )

         scores_df.write.format("delta").partitionBy("updated_at").mode("append").save(
            os.path.join(output_path, "bronze/scores_hist")
        )
    else:
        schedule_df.write.format("delta").mode("overwrite").save(
            os.path.join(output_path, "bronze/schedule")
        )
        scores_df.write.format("delta").mode("overwrite").save(
